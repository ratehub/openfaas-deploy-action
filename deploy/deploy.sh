...
then
    ...
    then
        faas-cli deploy --gateway="$FAAS_GATEWAY" --replace=true --update=false
    fi
else
    GROUP_PATH=""
    GROUP_PATH2=""
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        if [ "$GITHUB_EVENT_NAME" == "push" ];
                        then
                            faas-cli deploy --gateway="$FAAS_GATEWAY" --filter="$FUNCTION_PATH" --replace=true --update=false
                        fi
